...
from nnunetv2.inference.export_prediction import export_prediction_from_logits, export_uncertainty, \
...
class nnUNetPredictor(object):
    ...
    def predict_from_files(self,
        ...
        return self.predict_from_data_iterator(data_iterator, save_probabilities, num_processes_segmentation_export,nums_MC)
    ...
    def predict_from_data_iterator(self,
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                predict_list = []
                for i in range(nums_MC):
                    # apply random tta to estimate uncertainty
                    random_tta = 1 != nums_MC

                    # prediction shape: C,D,H,W
                    prediction = self.predict_logits_from_preprocessed_data(data,random_tta).cpu()
                    ...
                    if nums_MC > 1:
                        r.append(
                            export_pool.starmap_async(
                                    export_uncertainty,
                                    ((predict_list, properties, self.configuration_manager, self.plans_manager,
                                    self.dataset_json, ofile, save_probabilities),)
                            )
                        )
                    else:
    ...
    def predict_logits_from_preprocessed_data(self, data: torch.Tensor, random_tta: bool = False) -> torch.Tensor:
        """
        ...
        for params in self.list_of_parameters:
            ...
            if prediction is None:
                prediction = self.predict_sliding_window_return_logits(data, random_tta).to('cpu')
            else:
                prediction += self.predict_sliding_window_return_logits(data, random_tta).to('cpu')
        ...
        try:
            ...
            for sl in tqdm(slicers, disable=not self.allow_tqdm):
                ...
                if random_tta:
                    ...
                    prediction = augmentated_prediction.random_TTA_and_predict(workon)[0].to(results_device)
                else:
    ...
    def predict_sliding_window_return_logits(self, input_image: torch.Tensor, random_tta: bool = False) \
        ...
        with torch.autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            if self.perform_everything_on_device and self.device != 'cpu':
                ...
                try:
                    ...
                    empty_cache(self.device)
                    predicted_logits = self._internal_predict_sliding_window_return_logits(data, slicers, False,random_tta)
            else:
                predicted_logits = self._internal_predict_sliding_window_return_logits(data, slicers,
                                                                                       self.perform_everything_on_device,random_tta)
...
def predict_entry_point():
    ...
    parser.add_argument('--nums_MC', type=int, required=False, default=1,
                    help='Number of Monte Carlo simulation to estimate Aleatoric Uncertainty.')
