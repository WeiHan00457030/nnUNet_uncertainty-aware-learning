...
from nnunetv2.inference.augmentated_prediction import AugmentatedPrediction
...
class nnUNetTrainer(object):
    ...
    def on_train_epoch_start(self, lr_reset_epoch:int = None):
        self.network.train()
        self.lr_scheduler.step(self.current_epoch if lr_reset_epoch is None else lr_reset_epoch)
        self.print_to_log_file('')
    ...
    def train_step(self, batch: dict, weighted_uncertainty_loss: bool = False, weight: float = None, high_uncertainty: bool = None) -> dict:
        data = batch['data'] # [2,1,20,256,256]
        target = batch['target']
        ...
        augmentated_prediction = AugmentatedPrediction(self.network)
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            if weighted_uncertainty_loss:
                uncertainty_map = augmentated_prediction.estimate_uncertainty_map(data , scale_indexs=[0])
            else:
                uncertainty_map = [torch.empty(0).to(self.device) for _ in range(6)]

            # output len = 6
            # output[0].shape = [2,6,20,256,256]
            output = self.network(data)
           
            # del data
            l = self.loss(output, target, uncertainty_map, uncertainty_weight=weight, high_uncertainty=high_uncertainty)
            del uncertainty_map
    ...
    def on_epoch_end(self):
        ...
        self.print_to_log_file('ema_fg_dice',np.round(self.logger.my_fantastic_logging['ema_fg_dice'][-1], decimals=4))
        self.print_to_log_file(
        ...
        if (current_epoch + 1) % self.save_every == 0 and current_epoch != (self.num_epochs - 1):
            self.save_checkpoint(join(self.output_folder, f'checkpoint_latest_{current_epoch}.pth'))
    ...
    def run_training(self):
        ...
        for epoch in range(self.current_epoch, self.num_epochs):
            ...
            # dynamic add weight
            if epoch > start_uncertainty_epoch and epoch % 10 == 0:
                weight = min(max_uncertainty, weight + 1)

            # reset lr when start focusing high uncertainty
            if epoch == start_focusing_high_uncertainty:
                self.lr_scheduler.reset(self.initial_lr/3, self.num_epochs - start_focusing_high_uncertainty)
                self.print_to_log_file('start_focusing_high_uncertainty')

            # so that lr_scheduler could work
            if epoch >= start_focusing_high_uncertainty and epoch > start_uncertainty_epoch:
                self.on_train_epoch_start(epoch-start_focusing_high_uncertainty)
            else:
                self.on_train_epoch_start()
                 
            train_outputs = []
            for batch_id in range(self.num_iterations_per_epoch):
                # use_uncertainty_after_epochs N
                if epoch < start_uncertainty_epoch:
                    train_outputs.append(self.train_step(next(self.dataloader_train),weighted_uncertainty_loss=False))
                else:
                    if epoch >= start_focusing_high_uncertainty:
                        train_outputs.append(self.train_step(next(self.dataloader_train),weighted_uncertainty_loss=True,weight=weight,high_uncertainty = True))
                    else:
                        train_outputs.append(self.train_step(next(self.dataloader_train),weighted_uncertainty_loss=True,weight=weight,high_uncertainty = False))
