...
class MemoryEfficientSoftDiceLoss(nn.Module):
    ...
    def forward(self, x, y, loss_mask=None, uncertainty_map=None, weight:float = None, high_uncertainty:bool = None):
        if self.apply_nonlin is not None:
            x = self.apply_nonlin(x)
        ...
        if uncertainty_map is not None and uncertainty_map.numel() != 0:
            if high_uncertainty:
                x = x * (1 + weight * (uncertainty_map))
            else:
                x = x * (1 + weight * (1 - uncertainty_map))
