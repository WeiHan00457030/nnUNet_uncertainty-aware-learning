...
class DeepSupervisionWrapper(nn.Module):
    ...
    def forward(self, *args, uncertainty_weight:float = None, high_uncertainty:bool = None):
        ...
        if uncertainty_weight is not None:
            return sum([weights[i] * self.loss(*inputs, uncertainty_weight, high_uncertainty) for i, inputs in enumerate(zip(*args)) if weights[i] != 0.0])
        else:
